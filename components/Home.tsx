      ...
      <section className="relative py-12">
        <div className="max-w-4xl">
          ...
          <div className="flex flex-wrap gap-4">
            ...
            <button
              onClick={async () => {
                const { getZerodhaLoginUrl } = await import('../services/geminiService');
                const url = await getZerodhaLoginUrl();
                window.location.href = url;
              }}
              className="px-8 py-4 bg-orange-500 text-white rounded-2xl font-black text-sm hover:bg-orange-600 transition-all shadow-xl shadow-orange-100 flex items-center gap-2 hover:scale-105 active:scale-95"
            >
              Connect Zerodha
            </button>
            <button
